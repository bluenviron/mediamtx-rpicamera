...
bool encoder_create(const parameters_t *params, int stride, int colorspace, encoder_output_cb output_cb, encoder_t **enc) {
    *enc = malloc(sizeof(encoder_priv_t));
    ...
    if (hardH264) {
        ...
        bool res = encoder_hardware_h264_create(params, stride, colorspace, output_cb, &hardware_h264);
        if (!res) {
        ...
        bool res = encoder_software_h264_create(params, stride, colorspace, output_cb, &software_h264);
        if (!res) {
